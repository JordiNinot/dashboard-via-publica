...
async function getIndex(store) {
  try {
    const raw = await store.get(INDEX_KEY, { consistency: "strong" });
...
async function handler(req) {
  ...
  try {
    if (req.method === "GET" && action === "list") {
      return Response.json(await getIndex(store), { headers: CORS });
    }
    if (req.method === "GET" && action === "get" && key) {
      const buffer = await store.get(key, { type: "arrayBuffer" });
      if (!buffer) return new Response("Not found", { status: 404, headers: CORS });
      const index = await getIndex(store);
      const meta = index.find((f) => f.nom === key) || {};
      const ct = meta.mimeType || "application/octet-stream";
      return new Response(buffer, {
        headers: { ...CORS, "Content-Type": ct, "Content-Disposition": `inline; filename*=UTF-8''${encodeURIComponent(key)}` }
      });
    }
    if (req.method === "POST" && action === "chunk") {
      ...
      if (!chunkBlob) return new Response("No chunk", { status: 400, headers: CORS });
      const buffer = await chunkBlob.arrayBuffer();
      const chunkKey = `__chunks__/${key}/${idx}`;
      await store.set(chunkKey, buffer);
      return Response.json({ ok: true, index: idx, total }, { headers: CORS });
    }
    if (req.method === "POST" && action === "assemble") {
      const { nom, total, mimeType } = await req.json();
      if (!nom || !total) return new Response("Missing nom/total", { status: 400, headers: CORS });
      const buffers = [];
      for (let i2 = 0; i2 < total; i2++) {
        const chunkKey = `__chunks__/${nom}/${i2}`;
        const buf = await store.get(chunkKey, { type: "arrayBuffer", consistency: "strong" });
        if (!buf) return new Response(`Chunk ${i2} missing`, { status: 500, headers: CORS });
        buffers.push(buf);
      ...
      for (const buf of buffers) {
        final.set(new Uint8Array(buf), offset);
        offset += buf.byteLength;
      }
      await store.set(nom, final.buffer);
      const entry = { nom, mida: formatSize(totalBytes), data: (/* @__PURE__ */ new Date()).toLocaleDateString("ca-ES"), mimeType: mimeType || "application/octet-stream" };
      const index = await getIndex(store);
      const i = index.findIndex((f) => f.nom === nom);
      if (i >= 0) index[i] = entry;
      else index.push(entry);
      await saveIndex(store, index);
      return Response.json({ ok: true, mida: formatSize(totalBytes) }, { headers: CORS });
    }
    if (req.method === "POST" && !action) {
      ...
      const entry = { nom: file.name, mida: formatSize(file.size), data: (/* @__PURE__ */ new Date()).toLocaleDateString("ca-ES"), mimeType: file.type || "application/octet-stream" };
      const index = await getIndex(store);
      const i = index.findIndex((f) => f.nom === file.name);
      if (i >= 0) index[i] = entry;
      else index.push(entry);
      await saveIndex(store, index);
      return Response.json({ ok: true, key: file.name }, { headers: CORS });
    }
    if (req.method === "DELETE" && key) {
      await store.delete(key);
      const index = await getIndex(store);
      await saveIndex(store, index.filter((f) => f.nom !== key));
      return Response.json({ ok: true }, { headers: CORS });
    }
    return new Response("Method not allowed", { status: 405, headers: CORS });
  } catch (err) {
    console.error("[blobs]", err);
    return new Response(JSON.stringify({ error: err.message }), { status: 500, headers: { ...CORS, "Content-Type": "application/json" } });
  }
}
var config = { path: "/api/blobs" };
export {
  config,
  handler as default
};
